...
function interactive(){
	try {
		if(ENUNCIADO.length){
			$('#enun2').empty().append('<p id="entxt" class="enen2">'+ENUNCIADO+'</p>');
			try {
		...
		$('#enun2').empty().css({minHeight:0,height:0}); // no enunciado! (but leave border)
	}
}
...
function asignature(){
	try {
		// setup colors
		$('.medio,#enun,#enun2').addClass(ASIGNATURA.toLowerCase() + " bg");
		$('.tinyButton,.specialButton,.genericButton,.tinyAudioButton').addClass(ASIGNATURA.toLowerCase());

		// setup font-faces & sizes
		var ee = ETAPA_EDUCATIVA.toLowerCase();
		$('.medio,#entxt,#enun2,.tinyButton,.genericButton,.logo,.specialButton').addClass(ee);

		// setup logo
		var lh = parseInt($('.logo embed').css('height'));
		if(!lh || isNaN(lh))lh=parseInt($('.logo img').css('height'));
...
$(document).ready(function(){
	...
	$(window).on("scroll",function(e){$(window).scrollTop(0);e.preventDefault();e.stopImmediatePropagation();return false;});
	doAdjust();
